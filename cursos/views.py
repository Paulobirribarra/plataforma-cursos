...
def course_detail(request, pk):
    ...
    if not course.is_free:
        ...
        if code:
            try:
                discount_code = DiscountCode.objects.get(course=course, code__iexact=code)
...
def course_list(request):
    free_courses = Course.objects.filter(is_visible=True, is_free=True)
    general_courses = Course.objects.filter(is_visible=True, is_free=False)
    ...
    if request.method == 'POST':
        if 'save_course' in request.POST:
            if form.is_valid():
                ...
                course.save()
                form.save_m2m()
                messages.success(request, f"Curso {'actualizado' if pk else 'creado'} exitosamente.")
            ...
            if resource_form.is_valid():
                ...
                messages.error(request, "Por favor, corrija los errores en el formulario de recursos.")
            ...
            if discount_form.is_valid():
                ...
                messages.error(request, "Por favor, corrija los errores en el formulario de descuento.")
...
def course_resource_create(request, course_pk):
    ...
    return render(request, 'cursos/course_resource_form.html', {'form': form, 'course': course})
